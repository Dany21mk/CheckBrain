...
public class Level5GeogActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (FirebaseAuth.getInstance().getCurrentUser() == null){
            startActivity(new Intent(Level5GeogActivity.this, AuthActivity.class));
            finish();
        }
    ...
    public void handler() {
        rand = random.nextInt(array.australia.length);
        tv = findViewById(R.id.task);
        ans = array.australiaAnswer[rand];
        tv.setText(array.australia[rand]);
        ...
        for (int i = 0; i < answers.length; i++) {
            ...
            answers[i] = array.australiaAnswer[rand];
            ...
            while (count > 1){
                ...
                answers[i] = array.australiaAnswer[rnd];
                for (int j = 0; j < answers.length; j++) {
                    if (answers[i] == answers[j]){
                        count++;
                    }
